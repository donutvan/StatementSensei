...
from budget_sensei.rules import load_categories
from budget_sensei.services.categorization import recategorize_all_transactions
from budget_sensei.storage import ensure_data_files, init_db, load_transactions, save_override
...
def _reapply_model_controls() -> None:
    st.write("Re-run rules and the machine learning model across stored transactions.")
    st.write("Manual overrides remain unchanged.")
    if st.button("Reapply categorisation with model"):
        updated = recategorize_all_transactions()
        st.success(f"Reapplied categorisation to {updated} transactions.")
...
def main() -> None:
    ...
    st.subheader("Reapply machine learning categorisation")
    _reapply_model_controls()
