...
def ensure_data_files() -> None:
    ...
    if not labeled_path.exists():
        pd.DataFrame(columns=["transaction_hash", "description", "amount", "category"]).to_csv(
            labeled_path, index=False
        )
    if not overrides_path.exists():
        pd.DataFrame(columns=["transaction_hash", "category"]).to_csv(
...
def update_categories_bulk(mapping: pd.DataFrame) -> int:
    if mapping.empty:
        return 0
    required = {"transaction_hash", "category"}
    if not required.issubset(mapping.columns):
        raise ValueError("mapping must include transaction_hash and category columns")
    conn = connect_db()
    conn.register("category_updates", mapping[["transaction_hash", "category"]])
    conn.execute(
        """
        UPDATE transactions AS t
        SET category = u.category
        FROM category_updates AS u
        WHERE t.transaction_hash = u.transaction_hash
        """
    )
    changes = conn.execute("SELECT count(*) FROM category_updates").fetchone()[0]
    conn.close()
    return changes
...
def save_override(transaction_hash: str, category: str) -> None:
    ...
    if transaction_hash in set(overrides["transaction_hash"]):
        ...
        overrides = pd.concat(
            [
                overrides,
                pd.DataFrame({"transaction_hash": [transaction_hash], "category": [category]}),
            ],
            ignore_index=True,
        )
